...
import { Users, AlertTriangle, GraduationCap, Tag, CheckCircle, User, Search, AlertCircle, RefreshCw, BarChart2, PieChart as PieChartIcon, Calendar, Target, CalendarDays, ChevronUp } from "lucide-react";
import {
...
export function GroupsView({
  ...
  return (
    <div className="section-card">
      <h2 className="section-title" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><Users size={24} /> My Assigned Groups</h2>
      <p style={{ color: "#64748b", marginBottom: "1.5rem" }}>
        Manage the groups you are actively supervising. Evaluate their phases and check their code progress.
      </p>
      ...
      {groupsLoading ? (
        ...
        <div className="center-state">
          <div className="state-icon"><AlertTriangle size={32} /></div>
          <h3>Error</h3>
          <p>{groupsError}</p>
        ...
        <div className="center-state">
          <div className="state-icon"><Users size={32} /></div>
        ...
        <div className="groups-grid">
          {groups.map((g, i) => {
            ...
            const twoWeeksData = perf ? Object.keys(perf.twoWeekCommits || {}).map(user => ({
              name: user, Commits: perf.twoWeekCommits[user] || 0
            })) : [];
            const monthPieData = perf ? Object.keys(perf.monthCommits || {}).map((user, ci) => ({
              name: user, value: perf.monthCommits[user] || 0, fill: CHART_COLORS[ci % CHART_COLORS.length]
            })) : [];
            ...
            return (
              <div key={gId} className="group-card">
                <div className="group-card-header">
                  <div>
                    <h3 style={{ fontSize: '1rem', fontWeight: 700, margin: 0, color: '#1e293b', display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <GraduationCap size={18} /> {g.proposal?.title || g.name || `Group #${gId}`}
                    </h3>
                    {g.proposal?.domain && (
                      <span style={{
                      ...
                      }}><Tag size={12} style={{ display: 'inline', marginRight: '4px' }} /> {g.proposal.domain}</span>
                    )}
                  </div>
                  <button className="btn-primary" onClick={() => handleOpenEvaluate(gId, 'supervisor')} style={{ padding: '0.4rem 0.8rem', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                {/* --- SUPERVISOR SPECIFIC: Read & Write GitHub Section --- */}
                <div className="github-section">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                    ...
                    {repoUrl && !isAdding && (
                      <button onClick={() => { setAddingRepoFor(gId); setRepoInputUrl(repoUrl); }} style={{ background: 'transparent', border: 'none', color: '#3b82f6', fontSize: '0.75rem', cursor: 'pointer', fontWeight: 600, padding: 0 }}>
                        Edit Link
                      </button>
                    )}
                  </div>
                  ...
                  {isAdding ? (
                    <div style={{ background: '#f8fafc', padding: '1rem', borderRadius: '8px', border: '1px solid #e2e8f0', marginBottom: '1rem' }}>
                      <p style={{ fontSize: '0.75rem', color: '#64748b', margin: '0 0 8px', fontWeight: 600 }}>Enter Repository URL</p>
                      <input
                        type="text"
                        placeholder="https://github.com/username/repo"
                        value={repoInputUrl}
                        onChange={(e) => setRepoInputUrl(e.target.value)}
                        style={{ width: '100%', padding: '0.6rem', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '0.8rem', marginBottom: '10px' }}
                      />
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button onClick={() => handleUpdateRepo(gId)} style={{ background: '#3b82f6', color: '#fff', border: 'none', padding: '6px 12px', borderRadius: '6px', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer' }}>
                          Save
                        </button>
                        <button onClick={() => setAddingRepoFor(null)} style={{ background: '#e2e8f0', color: '#475569', border: 'none', padding: '6px 12px', borderRadius: '6px', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer' }}>
                          Cancel
                        </button>
                      </div>
                    </div>
                    ...
                    <div style={{ background: '#f8fafc', border: '1px dashed #cbd5e1', padding: '1rem', borderRadius: '8px', textAlign: 'center', marginBottom: '1rem' }}>
                      ...
                      <button className="btn-add-repo" onClick={() => { setAddingRepoFor(gId); setRepoInputUrl(''); }}>
                  ...
                  {!perf && repoUrl && !thisError && !isChecking && !isAdding && (
                    <button
                    ...
                    >
                      <Search size={14} style={{ marginRight: '6px' }} /> Check Code Performance
                    </button>
                  )}
                ...
                {thisError && !isChecking && (
                  <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '8px', padding: '1rem' }}>
                    <p style={{ color: '#dc2626', fontSize: '0.8rem', fontWeight: 700, margin: '0 0 6px', display: 'flex', alignItems: 'center', gap: '4px' }}><AlertCircle size={14} /> Error</p>
              ...
              <div>
                ...
                {totalCommitData.length > 0 && (
                  <div style={{ marginBottom: '1.25rem' }}>
                    <p style={{ fontSize: '0.68rem', fontWeight: 800, textTransform: 'uppercase', color: '#475569', letterSpacing: '0.5px', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '4px' }}><BarChart2 size={12} /> Total Commits</p>
                ...
                {pieData.length > 0 && (
                  <div style={{ marginBottom: '1.25rem' }}>
                    <p style={{ fontSize: '0.68rem', fontWeight: 800, textTransform: 'uppercase', color: '#475569', letterSpacing: '0.5px', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '4px' }}><PieChartIcon size={12} /> Commit Share</p>
                ...
                {weekMonthData.length > 0 && (
                  <div style={{ marginBottom: '1.25rem' }}>
                    <p style={{ fontSize: '0.68rem', fontWeight: 800, textTransform: 'uppercase', color: '#475569', letterSpacing: '0.5px', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '4px' }}><Calendar size={12} /> Last Week vs Month</p>
                    <div style={{ background: '#fff', borderRadius: '10px', padding: '0.75rem', border: '1px solid #f1f5f9' }}>
                      <ResponsiveContainer width="100%" height={160}>
                ...
                {consistencyData.length > 0 && (
                  <div>
                    <p style={{ fontSize: '0.68rem', fontWeight: 800, textTransform: 'uppercase', color: '#475569', letterSpacing: '0.5px', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '4px' }}><Target size={12} /> Consistency Score</p>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                ...
                                <span style={{ fontSize: '0.65rem', padding: '1px 7px', borderRadius: '100px', background: c.score >= 70 ? '#d1fae5' : c.score >= 40 ? '#fef9c3' : '#fee2e2', color, fontWeight: 700 }}>
                                  {c.score >= 70 ? 'Consistent' : c.score >= 40 ? 'Moderate' : 'Inconsistent'}
                                </span>
                              </div>
                            </div>
                ...
                {twoWeeksData.length > 0 && (
                  <div style={{ marginTop: '1.25rem', marginBottom: '1.25rem' }}>
                    <p style={{ fontSize: '0.68rem', fontWeight: 800, textTransform: 'uppercase', color: '#475569', letterSpacing: '0.5px', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '4px' }}><CalendarDays size={12} /> Last 2 Weeks Commits</p>
                    <div style={{ background: '#fff', borderRadius: '10px', padding: '0.75rem', border: '1px solid #f1f5f9' }}>
                      <ResponsiveContainer width="100%" height={160}>
                ...
                {monthPieData.length > 0 && (
                  <div style={{ marginBottom: '1.25rem' }}>
                    <p style={{ fontSize: '0.68rem', fontWeight: 800, textTransform: 'uppercase', color: '#475569', letterSpacing: '0.5px', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '4px' }}><Calendar size={12} /> Last Month Commits</p>
                    <div style={{ background: '#fff', borderRadius: '10px', padding: '0.75rem', border: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
                      <ResponsiveContainer width={150} height={150}>
                        <PieChart>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.875rem' }}>
                  ...
                  >
                          <RefreshCw size={12} style={{ marginRight: '4px' }} /> Re-evaluate
                </button>
                <button
                ...
                >
                          <ChevronUp size={12} style={{ marginRight: '4px' }} /> Show Less
              </button>
                      </div>
                    </div>
  )
